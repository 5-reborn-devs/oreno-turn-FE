...
public class PopupEveningDraw : UIBase
{
    ...
    public void OnClickItem(int cardData)
    {
        if (!isMyTurn) return;
        if (SocketManager.instance.isConnected)
        {
            GamePacket packet = new GamePacket();
            CardType cardType = (CardType)cardData;
            packet.EveningPickRequest = new C2SEveningPickRequest() { CardType = cardType };
